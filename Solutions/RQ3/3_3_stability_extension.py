...
def load_rq3_inputs_for_scenario(scenario_label: str, scenario_code: str) -> ScenarioBundle:
    ...
    return ScenarioBundle(
        ...
        satisfaction_summary=payload["satisfaction_best_summary"],
    ...
    if pareto_stats["count"] > 0:
        lines.extend(
            [
                ...
                "- 这说明论文展示的候选点中，大部分点本身就位于离散跳变更强的高收益/高满意度边界，"
...
def write_stability_notes(
    ...
    lines.extend(
        [
            "## 3. 不同可及绩效阈值下财务可持续性变化",
            "",
            "- 采用 epsilon-constraint 后，可以直接读取达到不同最低可及绩效阈值所需的最小财政缺口。",
    ...
    lines.extend(
        [
            ...
            "- 可以强调：S0 在当前布局下连 epsilon=0.4 都无法达到，说明较高最低可及绩效阈值不是简单追加补贴即可解决，而需要预算扩容或站点布局升级。",
            ...
            "",
            "- 不能写成“阻尼后模型全部收敛”。",
            "- 不能写成“当前政策已实现财务合规与高满意度/高可及绩效同时完全达标”；S4 只能写成“总利润率与较高可及绩效阈值可同时达到，但逐站利润率约束仍未全部满足”。",
