...
`full` 现在会先清空旧 `outputs/`、统一生图结果和 `Solutions/RQ4/cache/`，再按当前主链口径从头重算；因此它就是一键复现实验入口。
...
- `bash run_full_pipeline.sh full`
  - 会先清空旧 outputs、旧图片结果和 RQ4 cache，再按当前主链重算 RQ1 到 RQ4 的主结果与扩展结果，并在最后统一生图。
- `bash run_full_pipeline.sh plots`
...
6. `RQ3` 同步产出 `3_4_joint_feasibility_*` 逐站联合可行性诊断，以及 `3_5_satisfaction_objective_*` 站点—服务项目级扩展结果。
7. `RQ4` 按情景路径重求解 `RQ1` 至 `RQ3`，输出情景汇总、灵敏度系数、鲁棒性指标和解释备注。
8. `Plots` 统一读取 `RQ1` 至 `RQ4` 最新结果，生成论文图、图清单和图注建议。

说明：

- `run_full_pipeline.sh clean` 会同时清空 `Solutions/*/outputs/` 下的数值输出与图片类结果、`Solutions/RQ4/cache/` 和 `__pycache__`，但会保留各题 `outputs/README.md`。
- `RQ4` 会校验缓存是否仍符合当前主模型口径；若发现旧版 `alpha_j` 定价、溢出分流或过期摘要结构，会自动重算对应场景。
- `bash run_full_pipeline.sh full` 现在会先清缓存和旧结果，再在数值结果后自动生图；图表统一输出到 `Solutions/plots/outputs/`。
...
- 当前问题3主链路输出为 `3_1_best_price_scheme_*`；辅助扩展结果统一采用 `3_1_aux_financial_best_price_scheme_*`、`3_1_aux_satisfaction_best_price_scheme_*`、`3_1_aux_pareto_frontier.csv`、`3_2_aux_*`、`3_4_joint_feasibility_*` 与 `3_5_satisfaction_objective_*` 前缀，不覆盖主结果。
...
如果你现在要按当前主链口径一键重现论文结果，直接执行：

```bash
cd /Users/lifulin/Desktop/B

# 1. 从干净状态重跑 Q1 -> Q4 主结果、扩展结果，并自动统一生图
bash run_full_pipeline.sh full

# 2. 跑一遍守护测试，确认当前代码和结果链路一致
bash run_full_pipeline.sh test
```

若你只想先手动清空现有结果，再观察重算过程，可额外先执行：
...
- 各题输出说明：`Solutions/RQ1/outputs/README.md`、`Solutions/RQ2/outputs/README.md`、`Solutions/RQ3/outputs/README.md`、`Solutions/RQ4/outputs/README.md`
